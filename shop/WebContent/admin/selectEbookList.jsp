...
<body>
	...
	<br>
	<!-- 관리자 메뉴 인클루드 끝 -->
	<h1 style="text-align:center">Ebook List</h1>
	<br>
	<form action="<%=request.getContextPath()%>/admin/selectEbookList.jsp" style="text-align:right">
		...
		<button type="submit" class="btn btn-outline-dark btn-sm">전자책목록</button>
	</form>
	<br>
	<!-- 전자책 목록 출력 : 카테고리별 출력 -->
	<div>
		<table style="text-align:center" class="table table-striped">
			...
			<tbody>
				...
				<tr>
					<td><%=e.getEbookNo()%></td>
					<td><a href="<%=request.getContextPath()%>/admin/selectEbookOne.jsp?ebookNo=<%=e.getEbookNo()%>" class="btn btn-sm"><%=e.getEbookTitle()%></a></td>
		...
		<div style="text-align:center">
			...
			for (int i = startPage; i <= endPage; i++) {
				if (endPage <= lastPage) {
				%>
					<a class="btn btn-outline-dark" href="<%=request.getContextPath()%>/admin/selectEbookList.jsp?currentPage=<%=i%>&category=<%=categoryName%>"><%=i%></a>
				<%
			...
			%>
				<a class="btn btn-outline-dark" href="<%=request.getContextPath()%>/admin/selectEbookList.jsp?currentPage=<%=currentPage + 1%>&category=<%=categoryName%>">다음</a>
			<%
			}
			%>
			<a  class="btn btn-outline-dark" href="<%=request.getContextPath()%>/admin/selectEbookList.jsp?currentPage=<%=lastPage%>&category=<%=categoryName%>">끝으로</a>
		</div>
	</div>
</body>
</html>
